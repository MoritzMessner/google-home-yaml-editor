...
class OkGoogleStarter extends Starter {
  ...
  final String? for_; // Duration - checks if elapsed time since last valid event exceeded
  final String? suppressFor; // Duration - suppresses triggering for specified time period
}
...
class TimeScheduleStarter extends Starter {
  ...
  final List<String>? weekdays; // Array specifying applicable weekdays
}
...
class HomePresenceStarter extends Starter {
  ...
  final String? is_; // HOME, AWAY
  final String? isNot;
  final String? greaterThan;
  final String? greaterThanOrEqualTo;
  final String? lessThan;
  final String? lessThanOrEqualTo;
  final String? for_; // Duration - checks if state has remained for certain duration
  final String? suppressFor; // Duration - suppresses triggering for specified time period
}
...
enum StarterType {
  ...
  /// https://developers.home.google.com/automations/schema/reference/entity/sht_device/motion_detection_event
  motionDetectionEvent('Motion Detected (Event)', 'device.event.MotionDetection'),
  ...
  /// https://developers.home.google.com/automations/schema/reference/entity/sht_device/face_unfamiliar_detection_event
  faceUnfamiliarDetection('Unfamiliar Face', 'device.event.FaceUnfamiliarDetection'),

  /// https://developers.home.google.com/automations/schema/reference/entity/sht_device/animal_other_detection_event
  animalDetection('Animal Detected', 'device.event.AnimalOtherDetection'),

  /// https://developers.home.google.com/automations/schema/reference/entity/sht_device/moving_vehicle_detection_event
  movingVehicleDetection('Vehicle Detected', 'device.event.MovingVehicleDetection'),
  ...
  /// https://developers.home.google.com/automations/schema/reference/entity/sht_device/temperature_setting_state
  temperatureSettingState('Temperature Change', 'device.state.TemperatureSetting'),
  ...
  Starter createDefault() {
    return switch (this) {
      StarterType.okGoogle => const OkGoogleStarter(eventData: 'query', is_: 'Movie Night'),
      StarterType.timeSchedule => const TimeScheduleStarter(at: 'sunset'),
      StarterType.homePresence =>
        const HomePresenceStarter(state: 'homePresenceMode', is_: 'AWAY'),
      ...
      StarterType.onOffState =>
        const OnOffStateStarter(device: 'Light - Living Room', state: 'on', is_: true),
      StarterType.brightnessState =>
        const BrightnessStateStarter(device: 'Light - Living Room', state: 'brightness', is_: 50),
      StarterType.lockUnlockState =>
        const LockUnlockStateStarter(device: 'Front Door Lock', state: 'isLocked', is_: false),
      ...
      StarterType.temperatureSettingState => const TemperatureSettingStarter(
          device: 'Thermostat - Living Room', state: 'thermostatMode', is_: 'heat'),
      StarterType.motionDetectionState =>
        const MotionDetectionStateStarter(device: 'Sensor - Hallway', state: 'motionDetectionEventInProgress', is_: true),
      StarterType.occupancyState =>
        const OccupancyStateStarter(device: 'Sensor - Office', state: 'occupancy', is_: 'OCCUPIED'),
